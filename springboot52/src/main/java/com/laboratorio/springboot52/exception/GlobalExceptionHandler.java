...
import com.laboratorio.springboot52.dto.ErrorInfo;
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler({UsernameNotFoundException.class, InvalidUserExcepcion.class})
    public ResponseEntity<ErrorInfo> invalidUserError(Exception e) {
        ErrorInfo error = new ErrorInfo(6, e.getMessage());
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(error);
    }

    @ExceptionHandler({MailException.class, MessagingException.class})
    public ResponseEntity<ErrorInfo> sendEmailError(Exception e) {
        ErrorInfo error = new ErrorInfo(8, "Error enviando email");
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error);
    }
}